...
class TestBaseEvent:
    ...
    def test_base_event_creation_with_valid_data(self):
        ...
        # Act
        event = BaseEvent[dict](event_type=EventType.SYSTEM, source="test_service", data=test_data, timestamp=timestamp)
    ...
    def test_base_event_generic_typing(self):
        ...
        string_event = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test string")
        assert isinstance(string_event.data, str)
        assert string_event.data == "test string"

        # Arrange & Act - integer data
        int_event = BaseEvent[int](event_type=EventType.SYSTEM, source="test", data=42)
        assert isinstance(int_event.data, int)
        assert int_event.data == 42

        # Arrange & Act - list data
        list_event = BaseEvent[list](event_type=EventType.SYSTEM, source="test", data=[1, 2, 3])
    ...
    def test_base_event_auto_generated_fields(self):
        ...
        # Arrange & Act
        event1 = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test")
        event2 = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test")
    ...
    def test_base_event_timestamp_validation_timezone_aware(self):
        ...
        event1 = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", timestamp=naive_time)
        assert event1.timestamp.tzinfo == UTC
        assert event1.timestamp == naive_time.replace(tzinfo=UTC)

        # Act & Assert - UTC datetime preserved
        event2 = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", timestamp=utc_time)
        assert event2.timestamp == utc_time

        # Act & Assert - EST datetime converted to UTC
        event3 = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", timestamp=est_time)
    ...
    def test_base_event_timestamp_validation_errors(self):
        ...
        assert (
            ...
            or "input should be a valid datetime" in str(exc_info.value).lower()
        )
    ...
    def test_base_event_symbol_validation_normalization(self):
        ...
        event = BaseEvent[str](event_type=EventType.TRADE, source="test", data="test", symbol="  btcusdt  ")
        assert event.symbol == "BTCUSDT"

        # Act & Assert - None symbol (valid)
        event = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", symbol=None)
    ...
    def test_base_event_symbol_validation_errors(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            BaseEvent[str](event_type=EventType.TRADE, source="test", data="test", symbol="")
        ...
        with pytest.raises(ValidationError) as exc_info:
            BaseEvent[str](event_type=EventType.TRADE, source="test", data="test", symbol=123)
    ...
    def test_base_event_priority_validation(self):
        ...
        event = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test")
        assert event.priority == EventPriority.NORMAL

        # Act & Assert - custom priority
        event = BaseEvent[str](event_type=EventType.ERROR, source="test", data="test", priority=EventPriority.CRITICAL)
        assert event.priority == EventPriority.CRITICAL

        # Act & Assert - custom priority value
        custom_priority = EventPriority.custom(150)
        event = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", priority=custom_priority)
    ...
    def test_base_event_required_fields_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            BaseEvent[str](source="test", data="test")
        assert "Field required" in str(exc_info.value)

        # Act & Assert - missing source
        with pytest.raises(ValidationError) as exc_info:
            BaseEvent[str](event_type=EventType.SYSTEM, data="test")
        assert "Field required" in str(exc_info.value)

        # Act & Assert - missing data
        with pytest.raises(ValidationError) as exc_info:
            BaseEvent[str](event_type=EventType.SYSTEM, source="test")
        assert "Field required" in str(exc_info.value)
    ...
    def test_base_event_to_dict_method(self):
        ...
        # Arrange
        timestamp = datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC)
        event = BaseEvent[dict](event_type=EventType.TRADE, source="test", data={"price": 50000}, timestamp=timestamp)
    ...
    def test_base_event_string_representation(self):
        ...
        # Arrange
        event = BaseEvent[str](event_type=EventType.SYSTEM, source="test_service", data="test")
    ...
    def test_base_event_model_serialization(self):
        ...
        # Arrange
        event = BaseEvent[dict](
            event_type=EventType.KLINE, source="binance", data={"symbol": "BTCUSDT", "price": 50000}
        )
    ...
    def test_base_event_boundary_cases(self):
        ...
        long_source = "a" * 1000
        event = BaseEvent[str](event_type=EventType.SYSTEM, source=long_source, data="test")
        ...
        event = BaseEvent[dict](event_type=EventType.SYSTEM, source="test", data=complex_data)
    ...
    def test_base_event_correlation_id_validation(self):
        ...
        event = BaseEvent[str](
            event_type=EventType.SYSTEM, source="test", data="test", correlation_id="correlation-123-abc"
        )
        assert event.correlation_id == "correlation-123-abc"

        # Act & Assert - None correlation_id (valid)
        event = BaseEvent[str](event_type=EventType.SYSTEM, source="test", data="test", correlation_id=None)
